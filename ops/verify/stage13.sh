...
work_dir="work/stage13-verification"
...
write_report() {
  ...
  printf '{"result":"%s","phase":"%s","failureLine":%s,"commitSha":"%s","scope":"checkout-fulfillment-commitment-delivery-pricing","executionEnvironment":"%s","databaseE2E":"%s","browserE2E":"%s","stage1To12Regression":"executed-by-prior-workflow-gates","migrations":"clean-and-repeatable","securityPrivacyConcurrencyIdempotency":"%s","backupRestore":"%s","rpoSeconds":%s,"rtoSeconds":%s,"evidenceSha256":%s}\n' \
    "$result" "$phase" "$failure_line" "${GITHUB_SHA:-$(git rev-parse HEAD)}" "$execution_environment" "$db_e2e" "$browser_e2e" "$checks" "$checks" "$rpo" "$rto" "$evidence" > "$report_dir/stage13-verification-report.json"
  sha256sum "$report_dir/stage13-verification-report.json" > "$report_dir/stage13-verification-report.sha256"
}
...
cleanup() {
  ...
  if [[ "$result" -ne 0 ]]; then
    ...
    echo "Stage 13 verification failed during phase: $phase" >&2
...
phase=stage13-db-e2e
set +e
"${compose[@]}" run --rm -e NODE_ENV=test -e RUN_STAGE13_E2E=1 \
  -e STAGE13_FULFILLMENT_ENABLED=true -e PROCESSING_DISPATCH_ENABLED=false \
  -e MATCHING_DISPATCH_ENABLED=false -e FULFILLMENT_DISPATCH_ENABLED=false \
  -e AFTERCARE_DISPATCH_ENABLED=false -e FINANCE_DISPATCH_ENABLED=false \
  -e ORDERING_DISPATCH_ENABLED=false \
  api pnpm --filter @agat/api exec vitest run test/stage13.e2e.spec.ts --no-file-parallelism 2>&1 | tee "$work_dir/db-e2e.log"
...
set +e
STAGE13_PDF_FIXTURE="$PWD/$work_dir/synthetic.pdf" pnpm --filter @agat/web exec playwright test e2e/stage13.spec.ts 2>&1 | tee "$work_dir/browser-e2e.log"
browser_status="${PIPESTATUS[0]}"
set -e
[[ "$browser_status" -eq 0 ]] || exit "$browser_status"
browser_e2e=passed

phase=privacy-cache-audit
...
if grep -Eqi 'address|location_code|rule_id|draft_id|order_id|phone|object_key|signed|request_id' <<<"$metrics"; then exit 1; fi
if grep -Eqi 'Synthetic district|addressCiphertext|quarantine/[a-f0-9]|objects/[a-f0-9]|X-Amz-Signature|\+99800000015' "$work_dir/api.log" "$work_dir/web.log"; then exit 1; fi
audit_leaks="$("${compose[@]}" exec -T postgres psql -U agat -d agat_print -Atqc 'SELECT count(*) FROM "AuditEvent" WHERE metadata::text ~* $$(address|cipher|phone|object.?key|signed.?url|provider.?reference)$$')"
test "$audit_leaks" = 0
...
rules="$(psql -XAtqc 'SELECT count(*) FROM "FulfillmentTariffRule"')"
preferences="$(psql -XAtqc 'SELECT count(*) FROM "OrderDraftFulfillmentPreference"')"
snapshots="$(psql -XAtqc 'SELECT count(*) FROM "OrderFulfillmentSelectionSnapshot"')"
[[ "$rules" -gt 0 && "$preferences" -gt 0 && "$snapshots" -gt 0 ]]
duplicates="$(psql -XAtqc 'SELECT count(*) FROM (SELECT "orderId" FROM "OrderFulfillmentSelectionSnapshot" GROUP BY "orderId" HAVING count(*) > 1) x')"
[[ "$duplicates" == 0 ]]
invalid_fields="$(psql -XAtqc 'SELECT count(*) FROM "OrderFulfillmentSelectionSnapshot" WHERE (mode = $$PICKUP$$ AND ("locationCode" <> $$PICKUP$$ OR "addressCiphertext" IS NOT NULL)) OR (mode = $$DELIVERY$$ AND ("locationCode" = $$PICKUP$$ OR "addressCiphertext" IS NULL))')"
[[ "$invalid_fields" == 0 ]]
mismatch="$(psql -XAtqc 'SELECT count(*) FROM "Order" o JOIN "PriceQuote" q ON q.id=o."priceQuoteId" JOIN "PriceSnapshot" p ON p."orderId"=o.id JOIN "OrderFulfillmentSelectionSnapshot" s ON s."orderId"=o.id JOIN "FulfillmentTariffRule" r ON r.id=s."fulfillmentTariffRuleId" WHERE p."totalMinor"<>q."totalMinor" OR s."feeMinor"<>r."feeMinor"')"
[[ "$mismatch" == 0 ]]
SCRIPT
...
echo 'Stage 13 verification succeeded.'
