...
cleanup() {
  ...
  docker rm -f agat-processing-isolation-check agat-processing-timeout-check +    >/dev/null 2>&1 || true
...
"${compose[@]}" run --rm +  -e NODE_ENV=test +  -e RUN_STAGE3_E2E=1 +  -e PROCESSING_DISPATCH_ENABLED=false +  api pnpm --filter @agat/api exec vitest run test/stage3.e2e.spec.ts +    --no-file-parallelism

echo 'Verifying isolated processing runtime and a successful normalized result.'
printf '%s' +  'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=' +  | base64 --decode > "$work_dir/input"
PROCESSING_IMAGE=agat-processing:local +PROCESSING_SECCOMP_PROFILE="$PWD/ops/processing/seccomp.json" +PROCESSING_TIMEOUT_SECONDS=120 +  sh ops/processing/run-job.sh +    "$work_dir/input" "$work_dir/output.pdf" PNG
test -s "$work_dir/output.pdf"
head -c 5 "$work_dir/output.pdf" | grep -q '%PDF-'

docker create --name agat-processing-isolation-check +  --network none +  --read-only +  --cap-drop ALL +  --security-opt no-new-privileges:true +  --security-opt "seccomp=$PWD/ops/processing/seccomp.json" +  --pids-limit 64 +  --memory 768m +  --memory-swap 768m +  --cpus 1 +  --user 65532:65532 +  --tmpfs /tmp:rw,noexec,nosuid,nodev,size=256m,uid=65532,gid=65532 +  --entrypoint sleep +  agat-processing:local 30 >/dev/null
...
if docker run --rm --network none --entrypoint python3 agat-processing:local +  -c "import socket; socket.create_connection(('1.1.1.1', 53), 1)" +  >/dev/null 2>&1; then
  echo 'Processing container unexpectedly reached the network.' >&2
  exit 1
fi
if docker run --rm --network none --read-only --cap-drop ALL +  --security-opt no-new-privileges:true --user 65532:65532 +  --entrypoint touch agat-processing:local /root-write-test >/dev/null 2>&1; then
  echo 'Processing container root filesystem was writable.' >&2
  exit 1
fi

set +e
timeout --signal=KILL 1 docker run --rm --name agat-processing-timeout-check +  --network none --read-only --cap-drop ALL +  --security-opt no-new-privileges:true +  --security-opt "seccomp=$PWD/ops/processing/seccomp.json" +  --pids-limit 16 --memory 64m --cpus 0.25 +  --entrypoint sleep agat-processing:local 30 >/dev/null 2>&1
timeout_status=$?
set -e
...
audit_leaks="$("${compose[@]}" exec -T postgres psql -U agat -d agat_print -Atqc +  "SELECT count(*) FROM \"AuditEvent\" WHERE metadata::text ~* '(quarantine/|objects/|results/|filename|signed.?url|\\+998)'")"
test "$audit_leaks" = 0
